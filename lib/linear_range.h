...
#ifndef LIB_LINEAR_RANGE_H_
#define LIB_LINEAR_RANGE_H_
...
#define LINEAR_RANGE_INIT(_min, _step, _min_idx, _max_idx)                                         \
	{                                                                                          \
		.min = (_min), .step = (_step), .min_idx = (_min_idx), .max_idx = (_max_idx),      \
	}
 ...
 */
static inline uint32_t linear_range_group_values_count(const struct linear_range *r, size_t r_cnt)
{
	uint32_t values = 0U;
 ...
 */
static inline int linear_range_get_value(const struct linear_range *r, uint16_t idx, int32_t *val)
{
 ...
 * @retval -EINVAL If index is out of range.
 */
static inline int linear_range_group_get_value(const struct linear_range *r, size_t r_cnt,
 ...
 */
static inline int linear_range_get_index(const struct linear_range *r, int32_t val, uint16_t *idx)
{
	if (val < r->min) {
		*idx = r->min_idx;
		return -ERANGE;
	...
	if (r->step == 0U) {
		...
		*idx = r->min_idx + DIV_ROUND_UP((uint32_t)(val - r->min), r->step);
	}
 ...
 */
static inline int linear_range_group_get_index(const struct linear_range *r, size_t r_cnt,
					       int32_t val, uint16_t *idx)
{
	for (size_t i = 0U; i < r_cnt; i++) {
		if ((val > linear_range_get_max_value(&r[i])) && (i < (r_cnt - 1U))) {
			continue;
		}
 ...
 */
static inline int linear_range_get_win_index(const struct linear_range *r, int32_t val_min,
					     int32_t val_max, uint16_t *idx)
{
 ...
 * zero groups).
 */
static inline int linear_range_group_get_win_index(const struct linear_range *r, size_t r_cnt,
						   int32_t val_min, int32_t val_max, uint16_t *idx)
{
	for (size_t i = 0U; i < r_cnt; i++) {
